"""Publish Seru warehub APK and update the listing on zibashu4.com."""
...
APK_LOCAL = ANDROID_ROOT / "dist" / "seru-v1.0.0.apk"
ICON_LOCAL = (
    ANDROID_ROOT
    / "apps"
    / "seru"
...
def main() -> None:
    ...
    icon_name = f"seru-{ts}-{rand_token()}.png"
    ...
    payload = {
        ...
        "short_description": "Chat, friends, threads, and ZIBA pay — from ziBashu.",
        "description": (
            "Seru is ziBashu private messaging. Talk to friends, add or block people, "
            "read the public Thread feed, and pay with ZIBA receive codes. "
            "Message bodies stay on your device. Native Android — not a website wrapper."
        ),
        "category": "social",
        ...
        "platforms": platforms,
        "changelog": (
            f"v{VERSION_NAME}: Native Chat / Friends / Thread / Me. "
            "Friend-gated DMs with on-device bodies. ZIBA pay-via-code. "
            "Sign in with your ziBashu account (device code)."
        ),
        "tags": ["messaging", "chat", "ziba", "social"],
        "is_featured": 1,
        "is_active": 1,
        "sort_order": 15,
    }
    ...
    remote_payload = f"/tmp/seru-warehub-payload-{ts}.json"
...
"""
    remote_php = f"/tmp/seru-warehub-upsert-{ts}.php"
