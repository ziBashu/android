"""Publish KEYLINE warehub APK and listing on zibashu4.com."""
...
APK_LOCAL = ANDROID_ROOT / "dist" / "keyline-v1.0.0.apk"
ICON_LOCAL = ANDROID_ROOT / "apps" / "keyline" / "assets" / "icons" / "app_icon.png"
...
def main() -> None:
    ...
    icon_name = f"keyline-{ts}-{rand_token()}.png"
    ...
    payload = {
        ...
        "short_description": "Offline-first English keyboard. Typing stays on the device.",
        "description": (
            "KEYLINE is a quiet physical keyboard for Android. English QWERTY, "
            "numbers and symbols, long-press accents, basic local suggestions, "
            "and conservative autocorrect. No accounts, no cloud, no ads. "
            "What you type never leaves the device."
        ),
        "category": "tools",
        ...
        "platforms": platforms,
        "changelog": (
            "v1.0.0: First release. English QWERTY IME, number/symbol layer, "
            "shift/caps, long-press accents, local suggestions, conservative "
            "autocorrect, light/dark themes. Completely offline."
        ),
        "tags": ["offline", "keyboard", "ime", "tools"],
        "is_featured": 0,
        "is_active": 1,
        "sort_order": 40,
    }
    ...
    remote_payload = f"/tmp/keyline-warehub-payload-{ts}.json"
...
"""
    remote_php = f"/tmp/keyline-warehub-upsert-{ts}.php"
